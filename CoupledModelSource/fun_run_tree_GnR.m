function [hemo_save, geom_save] = fun_run_tree_GnR(s,params)

global connectivity diameter thickness length new_conn

%Derived from simulation settings
save_dir = "max" + s.max_days + "save" + s.save_steps + "dQ" + s.dQ + "dP" + s.dP + s.other + int2str(s.loop_iteration);
ts = 0: s.step_size: s.max_days / s.step_size - 1; %Time steps for the simulation
%dQ_s = s.dQ * (1 - exp( -s.k_ramp * ts)); %Ramping function for flow %* ones(size(ts));
dQ_s = s.dQ_s;
dP_s = s.dP * (1 - exp( -s.k_ramp * ts)); %Ramping function for pressure %* ones(size(ts));
...
while (diff > diff_max && count < count_max)
    order_geom_actual = calc_tree_equil_wgnr(order_geom_actual_temp, mat_tree, mat, n_orders, hemo, gamma_p_ord, gamma_q_ord, gamma_act_ord, cmd_flags, output_flags, params);
...
cmd_flags = "-r 0 -m " + s.max_days + " -s " + s.save_steps + " -d " + s.step_size + " -u 1 --simulate_equil 0";
output_flags = 2;
order_geom = calc_tree_equil_wgnr(order_geom_actual, mat_tree, mat, n_orders, hemo, gamma_p_ord, gamma_q_ord, gamma_act_ord, cmd_flags, output_flags, params);
...
if s.single_vessels_flag == 0
    ...
    while (diff(count) > diff_max && count < count_max)
        ...
        output_flags = 3;
        order_geom_perturb = calc_tree_equil_wgnr(order_geom_actual, mat_tree, mat, n_orders, hemo, gamma_p_ord, gamma_q_ord, gamma_act_ord, cmd_flags, output_flags, params);
    ...
    % -r 0 -m s.max_days -s save_days -u 1 --simulate_equil 0 --gamma_p gamma_p_ord(ord) --gamma_q gamma_q_ord(ord) -n ord + ord
    tic
    %Step through gnr time course
    for t = 1:(s.max_days / (s.save_steps * s.step_size) - 1)
        disp("G&R time: " + t * s.save_steps + " days")
        ...
        while (diff(count) > diff_max && count < count_max)
            ...
            cmd_flags = "-r 1 -m " + s.max_days + " -s " + s.save_steps + " -d " + s.step_size + " -u 1 --simulate_equil 0" + gnr_iter_flag + gnr_out_flag + s.mech_infl_flag;
            order_geom_new = calc_tree_equil_wgnr(order_geom_actual, mat_tree, mat, n_orders, hemo, gamma_p_ord, gamma_q_ord, gamma_act_ord, cmd_flags, output_flags, params);
        ...
        if t == (s.max_days / (s.save_steps * s.step_size) - 1)
            n_iter = 11;
...
%                     cmd_flags = "-r 1 -m " + s.max_days + " -s " + s.save_steps + " -d " + s.step_size + " -u 1 --simulate_equil 0" + gnr_iter_flag + gnr_out_flag + s.mech_infl_flag + mech_exp_flag;
%                     output_flags = 4;
...
%             end
    ...
    %--------------------------------------------------------------------------
        ...
        fig_num = 6;
        fig6 = plot_compare_gnr(s.max_days, n_orders, s.step_size, fig_num);
    ...
    nts_remain = (s.max_days / s.step_size) - 1;
    gnr_iter_flag = " --gnr_iter_flag 0";
    gnr_out_flag = " --gnr_out_flag 1";
    cmd_flags = "-r 1 -m " + s.max_days + " -s " + nts_remain + " -d " + s.step_size + " -u 1 --simulate_equil 0" + gnr_iter_flag + gnr_out_flag + s.mech_infl_flag;
    output_flags = 2;
    order_geom_new = calc_tree_equil_wgnr(order_geom_actual, mat_tree, mat, n_orders, hemo, gamma_p_ord, gamma_q_ord, gamma_act_ord, cmd_flags, output_flags, params);
