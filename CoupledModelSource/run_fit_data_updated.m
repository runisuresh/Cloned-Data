...
%We need a script that will run the overall coupled hemodynamic+G&R script
%(run_tree_gnr.m) and compare the outputs to the results gathered from the
%literature for the MCT and SuHX rat models. For this we have some steps:

%--------------------------------------------------------------------------
%1. Read in the data for the models from the spreadsheet file
%--------------------------------------------------------------------------
...
days_run = 40;
input_vals = {'Cardiac output (ml/min)','Cardiac index'};
wt_vals = {'WT (%), <100 um OD','WT (%), >100 um OD'};
stiff_vals = {'Shear mod (kPa), <100 um OD','Shear mod (kPa), >100 um OD'};
pressure_vals = {'mPAP (mmHg)', 'RVSP (mmHg)'};
ecs_vals = {'Elastase activity ratio to control','Colagenase activity ratio to control','COL1A1 mRNA'};
metrics = [wt_vals,stiff_vals,'mPAP (mmHg)',ecs_vals,input_vals];
excel_data = cell(1,(numel(metrics) - numel(input_vals)));
co_data = [];
...
for metric = 1:numel(metrics)
    ...
    for sheet = 1:numel(sheets)
        ...
        if any(strcmp(curr_metric,input_vals))
...
simulation_settings.dQ_s = vq2 * 0;


curr_parameter = [0.1, 0.2, 0.4, 0.8, 1.6]; % Ki_steady
% curr_parameter = [2,4,8,16,32]; % gamma_i
num_params = numel(curr_parameter);
% num_params = 1;
...
for val = 1:num_params
    params_uniform_infl.Ki_steady = curr_parameter(val);
    ...
    % UPDATE HERE
    for metric_number = 1:(numel(metrics) - numel(input_vals))
        figure (7 + metric_number)

        if metric_number == (numel(metrics) - numel(input_vals))
...
% y_exp_pressure_fold = y_exp_pressure_fold';
% pressure_no_nan = rmmissing([y(:,5) y_exp_pressure_fold]);
% pressure_sim = pressure_no_nan(:,1);
% pressure_exp = pressure_no_nan(:,2);
% 
% J_pressure = ((pressure_sim - pressure_exp) / pressure_sim)
